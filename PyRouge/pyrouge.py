...
if __name__ == '__main__':
    ...
    root_dir = '../../Multi-document-extraction-based-Summarization/summaries/page_rank/'
    dataset_dir= '../../Multi-document-extraction-based-Summarization/dataset/Test_Summaries/'
    for i in ["1","17","20","22","24","27","29","31","36","38","44"]:
        file1= root_dir+i+".py"
        if len(i)==1:
            file2= dataset_dir +"d3000"+ i +"t/D3000"+i+".M.100.T.D"
        else:
             file2= dataset_dir +"d300"+ i +"t/D300"+i+".M.100.T.B"
        file.write(file1)
        file.write("\n")
        with open(file1) as f:
            para = f.readlines()
            para=para[0]
            text=para[10:len(para)-2]
            #print(text)
            r = Rouge()                
            system_generated_summary = text
        with open(file2) as f:
            para=f.readlines()
            #print(para)
            text= " ".join(para)
            manual_summmary = text
        file.write(str(r.rouge_l([system_generated_summary], [manual_summmary])))
        file.write("\n")
